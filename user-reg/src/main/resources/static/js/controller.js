...
app
		.controller(
				...
				function($scope, $http, $location, $route) {
					...
					$http({
						method : 'GET',
						url : 'http://localhost:8011/api/user/'
					}).then(function(response) {
					...
					$scope.deleteUser = function(userId) {
						$http({
							method : 'DELETE',
							url : 'http://localhost:8011/api/user/delete/' + userId
						})
								.then(
										function(response) {
...
app
		.controller(
				...
				function($scope, $http, $location, $routeParams, $route) {
					...
					$http({
						method : 'GET',
						url : 'http://localhost:8011/api/user/' + $scope.userId
					}).then(function(response) {
					...
					$scope.submitUserForm = function() {
						$http({
							method : 'PUT',
							url : 'http://localhost:8011/api/user/' + $routeParams.id,
							data : $scope.user,
						})
								.then(
										function(response) {
